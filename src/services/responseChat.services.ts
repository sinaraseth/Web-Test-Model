    ...
    if (hasPipes) {
      ...
      if (tableStartIndex >= 0) {
        ...
        if (dataLines.length > 0) {
          ...
          // Build HTML table
          let tableHtml =
            "<table style='margin: 1em 0; width: max-content; border-collapse: collapse; border: 1px solid #ddd;'>\n";
    ...
    return parts
      .map((part) => {
        ...
        if (part.trim().startsWith("<table")) {
              ...
              (match, attrs) => {
                ...
                if (hasStyle) {
                  return match.replace(
                    /style='([^']*)'/,
                    "style='$1 border-collapse: collapse; border: 1px solid #ddd;'",
                  );
                } else {
          ...
          styledTable = styledTable.replace(/<th([^>]*)>/g, (match, attrs) => {
            const styleMatch = attrs.match(/style='([^']*)'/);
            if (styleMatch) {
              const existingStyles = styleMatch[1];
              const newAttrs = attrs.replace(
                /style='[^']*'/,
                `style='${existingStyles} padding: 8px; border: 1px solid #ddd; background-color: #f5f5f5;'`,
              );
              return `<th${newAttrs}>`;
            } else {
          ...
          styledTable = styledTable.replace(/<td([^>]*)>/g, (match, attrs) => {
            const styleMatch = attrs.match(/style='([^']*)'/);
            if (styleMatch) {
              const existingStyles = styleMatch[1];
              const newAttrs = attrs.replace(
                /style='[^']*'/,
                `style='${existingStyles} padding: 8px; border: 1px solid #ddd;'`,
              );
              return `<td${newAttrs}>`;
            } else {
...
export const submitChatMessage = async (
  ...
  if (selectedModel === "Qwen3VL") {
    throw new Error(
      "Qwen3VL model is not available yet. Please select DeepSeek OCR or Paddle OCR instead.",
    );
  }
  ...
  try {
    ...
    if (htmlContent && typeof htmlContent === "string") {
      ...
      const htmlCodeFenceMatch = htmlContent.match(
        /```html\s*([\s\S]*?)\s*```/,
      );
      if (htmlCodeFenceMatch) {
        ...
        const genericCodeFenceMatch = htmlContent.match(
